...
public class GitLabOptions implements MutableDataSetter {
    ...
    public GitLabOptions(DataHolder options) {
        ...
        imageExtensions = GitLabExtension.IMAGE_EXTENSIONS.get(options);
        imageExtensionSet = new HashSet<>();
        String[] extensionsImg = imageExtensions.split(",");
        for (String ext : extensionsImg) {
            String trimmed = ext.trim();
            if (!trimmed.isEmpty()) {
                imageExtensionSet.add(trimmed);
            }
        }
    }
